...
num_atom = (dAtip/np.power(lc_pt,2))*2
dAtip_atom = (dAtip/np.power(lc_pt,2))*np.power(raddi_pt,2)*2
...
while gap < gapmax:
    ...
    for i in range(0, row): # loop for each bumpy
        ...
        if igap <= cutoff: # [nm]
            ...
            ac_conductance = ac_conductance + dAtip_atom*num_atom*liner_interpolate(igap) # [W/K] ([W/m2K]*[m2])
        else:
            nc_conductance = nc_conductance + dAtip_atom*num_atom*liner_interpolate(igap) # [W/K] ([W/m2K]*[m2])

        total_conductance = total_conductance + dAtip_atom*num_atom*liner_interpolate(igap) # [W/K] ([W/m2K]*[m2])
        total_area = total_area + dAtip_atom
