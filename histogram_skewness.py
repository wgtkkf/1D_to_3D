...
# factor to convert AFM raw data to Height data
...
# statistical values
mean_value = statistics.mean(data_1d_8107) # mean value of raw data
...
pdf_afm = norm.pdf(x,loc=param[0], scale=param[1])                                      # gaussian fitting with parameters for AFM
##pdf_bumpy = norm.pdf(x,loc=param_bumpy[0], scale=param_bumpy[1])                      # gaussian fitting with parameters for numerical bumpies
pdf_skewbumpy = skewnorm.pdf(x, a = skewness, loc=param_bumpy[0], scale=param_bumpy[1]) # skewed gaussian fitting with parameters for numerical bumpies
plt.plot(x, pdf_afm, 'r-', label="Gaussian (AFM bumpy)")                                      # plot gaussian fitting with parameters
##plt.plot(x, pdf_bumpy, 'b-', linestyle='dashed', label="Gaussian (Numerical bumpy)")        # plot gaussian fitting with parameters
plt.plot(x, pdf_skewbumpy, 'g-', linestyle='dashed', label="Skewed gaussian (Numerical bumpy)") # plot skewed gaussian fitting with parameters
plt.hist(data_1d_zsub, normed=True, color = 'red', label = 'AFM bumpy')             # histgram of AFM data, default setting
plt.hist(bumpy_1d, normed=True, alpha=.7,color = 'green', label = 'Skewed numerical bumpy') # histgram of numerical bumpies, default setting

## graph information
plt.title('Distribution', **csfont) # graph title
plt.xlabel('Height of bumpy [nm]', fontdict=None, labelpad=None, **csfont)
