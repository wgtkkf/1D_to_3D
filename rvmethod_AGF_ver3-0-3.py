...
while gap < gapmax:
    ...
    for i in range(0, nxmax+1): # x direction
        for j in range(0, nymax+1): # y direction
            ...
            #bumpy = vfactor*normal(para_ave, 6.0) # average & standard deviation
            #bumpy = vfactor*weibull_min.rvs(c=1.5,loc=0.2,scale=1.5) # average & standard deviation
            #bumpy = vfactor*normal(para_ave,2) # average & standard deviation
            #bumpy = vfactor*np.random.triangular(-2.5, 0.0, 2.5, size=None)
            #bumpy = np.random.uniform(-5.0, 5.0, size=None) # square
            #bumpy = np.random.standard_t(1.5, size=None)
            bumpy =  vfactor*exponweib.rvs(a=3.5, c=1.2, size=1) # weib
