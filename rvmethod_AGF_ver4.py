...
f2 = open('ave.txt', 'r') # read mode
f3 = open('std.txt', 'r') # read mode
...
for line2 in f2:
    #print(str(line2))
    para_ave = float(line2)

for line3 in f3:
    #print(str(line3))
    para_std = float(line3)

for l in range(0, nmax): # file number
    f4 = open("../surface/bumpy/data"+str(counter)+".txt","w")
    f5 = open("../surface/conductance/data"+str(counter)+".txt","w")
    f6 = open("../surface/area/data"+str(counter)+".txt","w")
    ...
    while gap < gapmax:
        ...
        for i in range(0, nxmax+1): # x direction
            for j in range(0, nymax+1): # y direction
                ...
                bumpy = np.random.standard_t(3, size=None)

                # Output with text files
                f4.write(str(lx/ucnano)) # x [nm]
                f4.write(str(' '))
                f4.write(str(ly/ucnano)) # y [nm]
                f4.write(str(' '))
                f4.write(str(bumpy)) # roughness [nm]
                f4.write('\n')
                ...
                for k in range(0, int((division-1)*0.5)):
                    ...
                    total_conductance = total_conductance + area*liner_interpolate(igap)
