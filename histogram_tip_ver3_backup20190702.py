...
for i in range(0,row*col):
    data_1d_zsub_temp[i] = vfactor*(data_1d_tip[i] - zshift)/ucnano # [nm], convert AFM raw data to Height data

#
data_1d_zsub = data_1d_zsub_temp
#print(str(data_1d_zsub))
...
#print(str(sf))
#print(str(param[1]))
pdf_afm = (1-sf)*norm.pdf(x,loc=param[0], scale=param[1]) # gaussian fitting with parameters

## for raw data histogram
weights = np.ones_like(data_1d_zsub)/len(data_1d_zsub)
bin_row = int(np.sqrt(row))
#print(str(bin_row))

## for numerical data histogram
weights_numerical = np.ones_like(bumpy_1d)/len(bumpy_1d)

plt.plot(x, abs(pdf_afm), color = 'black', label="Gaussian fitting") # display gaussian fitting with parameters
plt.hist(data_1d_zsub, weights=weights,bins=bin_row, color = 'red', label = 'Measured roughness') # histgram of AFM data, default setting
